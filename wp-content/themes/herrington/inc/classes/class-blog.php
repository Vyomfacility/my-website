<?php
if (!class_exists('Herrington_Blog')) {
    class Herrington_Blog
    {

        public function get_archive_meta($post_id = 0) {
            $archive_category = herrington()->get_theme_opt( 'archive_category', true );
            $archive_comment = herrington()->get_theme_opt('archive_comment', true);
            $archive_author = herrington()->get_theme_opt( 'archive_author', true );
                    ...
                    <div class="meta-inner ">
                        <?php if($archive_author) : ?>
                            <span class="post-author  ">
                                ...
                                <span><?php echo esc_html__('By', 'herrington'); ?> <?php the_author_posts_link(); ?></span>
                            </span>
                        <?php endif; ?>
                        ...
                        <?php if($archive_comment) : ?>
                            <span class="post-comments  ">
                                <a href="<?php echo get_comments_link($post_id); ?>">
                                    ...
                                    <span><?php comments_number(esc_html__('No Comments', 'herrington'), esc_html__(' 1 Comment', 'herrington'), esc_html__('%  Comments', 'herrington'), $post_id); ?></span>
                                </a>
                            </span>
                        <?php endif; ?>
                    </div>
        ...
        public function get_excerpt_more( $length = 55, $post = null ) {
            ...
            if ( post_password_required( $post ) ) {
                return esc_html__( 'Post password required.', 'herrington' );
            }

            $content = apply_filters( 'the_content', strip_shortcodes( $post->post_content ) );
            $content = str_replace( ']]>', ']]&gt;', $content );

            $excerpt_more = apply_filters( 'herrington_excerpt_more', '&hellip;' );
        ...
        public function get_post_metas(){
            $post_author = herrington()->get_theme_opt('post_author', true);
            $post_date = herrington()->get_theme_opt('post_date', true);
            $post_comment = herrington()->get_theme_opt('post_comment', true);
            $post_categorie = herrington()->get_theme_opt('post_categorie', true);
            if ($post_author || $post_date || $post_categorie || $post_comment) : ?>
                <div class="post-metas">
                    ...
                    <?php if($post_categorie && has_category()) : ?>
                    ...
                    <?php if($post_comment) : ?>
                        <span class="post-comments  align-items-center">
                            ...
                            <a href="<?php comments_link(); ?>">
                                <span><?php comments_number(esc_html__('No Comments', 'herrington'), esc_html__(' 1 Comment', 'herrington'), esc_html__('%  Comments', 'herrington')); ?></span>
                            </a>
                        </span>
                    <?php endif; ?>
                </div>
    ...
    public function herrington_set_post_views( $postID ) {
    ...
    public function get_post_share($post_id = 0) {
        ...
        $share_icons = herrington()->get_theme_opt( 'post_social_share_icon', [] );
        $social_facebook = herrington()->get_theme_opt( 'social_facebook', [] );
        $social_twitter = herrington()->get_theme_opt( 'social_twitter', [] );
        $social_pinterest = herrington()->get_theme_opt( 'social_pinterest', [] );
        $social_linkedin = herrington()->get_theme_opt( 'social_linkedin', [] );
        if($post_social_share != '1') return;
        $post = get_post($post_id);
        if ($post_social_share) {
            ?>
            <div class="post-shares align-items-center">
                <span class="label"><i class="fas fa-share-alt"></i> <?php echo esc_html__('Share', 'herrington'); ?> </span>
                <div class="social-share">
                    <div class="social">
                        <?php if($social_facebook): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Facebook', 'herrington'); ?>" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode(get_the_permalink($post_id)); ?>">
                                <i class="caseicon-facebook"></i>
                            </a>
                        <?php endif; ?>
                        <?php if($social_twitter): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Twitter', 'herrington'); ?>" target="_blank" href="https://twitter.com/intent/tweet?original_referer=<?php echo urldecode(home_url('/')); ?>&url=<?php echo urlencode(get_the_permalink($post_id)); ?>&text=<?php the_title();?>%20">
                                <span class="caseicon-twitter"></span>
                            </a>
                        <?php endif; ?>
                        <?php if($social_linkedin): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Linkedin', 'herrington'); ?>" target="_blank" href="https://www.linkedin.com/cws/share?url=<?php echo urlencode(get_the_permalink($post_id));?>">
                                <i class="caseicon-linkedin"></i>
                            </a>
                        <?php endif; ?>
                        <?php if($social_pinterest): ?>
                            <a class="pxl-icon " title="<?php echo esc_attr__('Pinterest', 'herrington'); ?>" target="_blank" href="https://pinterest.com/pin/create/button/?url=<?php echo urlencode(get_the_post_thumbnail_url($post_id, 'full')); ?>&media=&description=<?php echo urlencode(the_title_attribute(array('echo' => false, 'post' => $post))); ?>">
    ...
    public function get_post_nav() {
        $post_navigation = herrington()->get_theme_opt( 'post_navigation', false );
                ...
                <div class="nav-next-prev prev col relative text-start">
                    <div class="nav-inner">
                        ...
                        <div class="nav-label-wrap  align-items-center">
                            ...
                            <span class="nav-label"><?php echo esc_html__('Previous Post', 'herrington'); ?></span>
                        </div>
                ...
                <div class="nav-next-prev next col relative text-end">
                    <div class="nav-inner">
                        ...
                        <div class="nav-label-wrap  align-items-center justify-content-end">
                            <span class="nav-label"><?php echo esc_html__('Newer Post', 'herrington'); ?></span>
    ...
    public function get_project_nav() {
        ...
        $previous = ( is_attachment() ) ? get_post( $post->post_parent ) : get_adjacent_post( false, '', true );
        $next     = get_adjacent_post( false, '', false );
        $link_grid = herrington()->get_theme_opt( 'link_grid', '' );
        if ( ! $next && ! $previous )
            return;
        ?>
        <?php
    ...
    public function get_related_post(){
        $post_related_on = herrington()->get_theme_opt( 'post_related_on', false );
        ...
        if($post_related_on) {
            ...
            if (count($query_similar->posts) > 1) {
                ...
                wp_enqueue_script( 'herrington-swiper' );
                $opts = [
                ...
                <div class="pxl-related-post">
                    <h3 class="widget-title"><?php echo esc_html__('Related Posts', 'herrington'); ?></h3>
                        ...
                        <div class="pxl-related-post-inner pxl-swiper-wrapper swiper-wrapper">
                            <?php foreach ($query_similar->posts as $post):
                                ...
                                if (has_post_thumbnail(get_the_ID()) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)) :
                                    $thumbnail_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'herrington-blog-small', false);
                            endif;
