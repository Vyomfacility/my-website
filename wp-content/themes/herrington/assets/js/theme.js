; (function ($) {
    ...
    $(document).ready(function () {
        ...
        document.querySelectorAll(".pxl-column-line-yes").forEach((el) => {
            if (el.dataset.linesAdded === "1") return;
            el.dataset.linesAdded = "1";

            for (let i = 1; i <= 3; i++) {
                const span = document.createElement("span");
                span.className = `line-column line-column-${i}`;
                el.appendChild(span);
            }
        });
    ...
    function herrington_circle_svg(element, linearGradientId, linearGradientId1) {
        ...
        const dot1 = s.circle(0, 0, size).attr({
            class: 'dot',
            fill: 'url(#' + linearGradientId + ')'
        });


        function setDot(dot, l) {
            const p = path.getPointAtLength(l);
            dot.attr({ cx: p.x, cy: p.y });
        }
