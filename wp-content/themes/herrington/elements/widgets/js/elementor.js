(function ($) {
    ...
    function pxl_scroll_list_iconbox() {

        $(".pxl-icons-list").each(function (index) {
            if (window.innerWidth < 1201) {
                return;
            }
            const parent = this;
            const el = parent.querySelector(".pxl-image-scroll-bg");
            if (!el) return;
            ScrollTrigger.getAll().forEach(st => {
                if (st.vars && st.vars.id === "pxl-scroll-iconbox-" + index) {
                    st.kill();
                }
            });

            const parentHeight = parent.offsetHeight;
            const elHeight = el.offsetHeight; // 100vh

            ScrollTrigger.create({
                id: "pxl-scroll-iconbox-" + index,
                trigger: parent,
                start: "top top",
                end: () => "+=" + (parentHeight - elHeight),
                pin: el,
                pinSpacing: false,
                scrub: false,
                anticipatePin: 1
    ...
    function pxlIconsList1Active() {
        const $items = $('.pxl-icons-list1 .list-item .pxl-iconbox-item');
    ...
    function pxl_min_height_testimonial_desc($scope) {
        const $carousels = $scope
            ? $scope.find('.pxl-testimonial-carousel5')
            : $('.pxl-testimonial-carousel5');

        if (!$carousels.length) return;

        $carousels.each(function () {
            const $descs = $(this).find('.pxl-swiper-slide .pxl-item--desc');
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        pxl_scroll_list_iconbox();
        pxlIconsList1Active();
        pxl_min_height_testimonial_desc();
