...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-list">'.esc_html__( 'No Post Found', 'donalfarm' ). '</div>';
    return;
}
?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    ...
    <div class="pxl-list-inner d-flex-wrap relative"> 
        <?php donalfarm_get_post_list($posts, $load_more); ?>
    </div>
     
    <?php if ($pagination_type == 'pagination') { ?>
        <div class="pxl-grid-pagination pagin-post d-flex">
            <?php donalfarm()->page->get_pagination($query, true); ?>
        </div>
    <?php } ?>
    <?php if (!empty($next_link) && $pagination_type == 'loadmore'): 
        ...
        ?>
        <div class="pxl-load-more d-flex" data-loading-text="<?php echo esc_attr__('Loading', 'donalfarm') ?>" data-loadmore-text="<?php echo esc_html($settings['loadmore_text']); ?>">
