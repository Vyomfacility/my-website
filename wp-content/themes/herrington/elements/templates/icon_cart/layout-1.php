<?php if ( class_exists( 'Woocommerce' ) ) { ?>
	<div class="pxl-cart-sidebar-button <?php echo esc_attr($settings['style']); ?>">
		<?php if(!empty($settings['pxl_icon']['value'])) {
			...
			<?php } ?>
			<?php if(class_exists('Woocommerce')) : ?>
				<span class="pxl_cart_counter"><?php echo sprintf (_n( '%d', '%d', WC()->cart->cart_contents_count, 'herrington' ), WC()->cart->cart_contents_count ); ?></span>
			<?php endif; ?>
		</div>
	<?php }
		if ( class_exists( 'Woocommerce' ) ) { add_action( 'pxl_anchor_target', 'herrington_hook_anchor_cart'); } ?>