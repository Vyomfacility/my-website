<?php
if (! defined('ABSPATH')) {
    exit();
}
/**
 * Add Bravis User Core.
 *
 * @name Bravis User_Ajax
 * @since 1.0.0
 */
if (! class_exists('Bravis_User_Ajax')) {

    class Bravis_User_Ajax
    {

        function __construct() {
            
            /* action login */
            add_action('wp_ajax_nopriv_pxl_user_login', array(
                $this,
                'form_ajax_login_callback'
            ));
             
            /* action register */
            add_action('wp_ajax_nopriv_form_ajax_register', array(
                $this,
                'form_ajax_register_callback'
            ));
        }

        /**
         * Register callback
         *
         * @package Bravis User*/
        function form_ajax_register_callback() {
            header('Content-Type: application/json');
            check_ajax_referer( Bravis_User::NONCE );
            /* if login data null. */
            if( empty($_POST['data']) )
                die(json_encode((object)array('error'=> true, 'message' => esc_html__('Request Null', 'bravis-user'))));

            $register_data = $_POST['data'];
            $register_data['user']  = sanitize_user($register_data['user']);
            $register_data['pass']  = esc_attr($register_data['pass']);
            $register_data['email'] = sanitize_email($register_data['email']);

            /* check user null */
            if( empty($register_data['user']) )
                die(json_encode((object)array('error'=> true, 'user_null' => esc_html__('User name null', 'bravis-user'))));
            /* check pass null */
            if( empty($register_data['pass']) )
                die(json_encode((object)array('error'=> true, 'pass_null' => esc_html__('Password null', 'bravis-user'))));
            /* check email null */
            if( empty($register_data['email']) )
                die(json_encode((object)array('error'=> true, 'email_null' => esc_html__('Email null', 'bravis-user'))));
            /* check valid email */
            if( $register_data['passconfirm'] != $register_data['pass'] )
                die(json_encode((object)array('error'=> true, 'passconfirm' => esc_html__('Password and confirmation password do not match.', 'bravis-user'))));

            /* check valid user name */
            if ( ! validate_username( $register_data['user'] ) )
                die(json_encode((object)array('error'=> true, 'user_invalid' => esc_html__('The username is not valid!', 'bravis-user'))));
            /* check user name exists */
            if ( username_exists( $register_data['user'] ) )
                die(json_encode((object)array('error'=> true, 'user_exists' => esc_html__('Username already exists!', 'bravis-user'))));
            /* check email already registed */
            if ( email_exists( $register_data['email'] ) )
                die(json_encode((object)array('error'=> true, 'email_exists' => esc_html__('Email Already in use.', 'bravis-user'))));
            if( $this->register_action($register_data['user'], $register_data['pass'], $register_data['email']) )
                die( json_encode((object)array('error'=> false)) );
            exit();
        }

        /**
         * To register
         *
         **/
        function register_action( $user, $pass, $email ) {
            $userdata = array(
                'user_login' => $user,
                'user_pass'  => $pass,
                'user_email' => $email
            );

            if( wp_insert_user( $userdata ) ) {
                /* check user & pass */
                if (! wp_login($user, $pass))
                    return false;

                /* get user by name. */
                $user = get_user_by('slug', $user);

                /* set login. */
                wp_set_auth_cookie( $user->data->ID, false, false );
                return true;
            }
            return false;
        }

        /**
         * Login callback.
         *
         * @package Bravis User
         */
        function form_ajax_login_callback()
        {
            header('Content-Type: application/json');
            check_ajax_referer( Bravis_User::NONCE );
            /* if login data null. */
            if(empty($_POST['data']))
                die(json_encode((object)array('error'=> true, 'message' => esc_html__('Request Null', 'bravis-user'))));

            $login_data = $_POST['data'];

            /* if user null. */
            if(empty($login_data['user']))
                die(json_encode((object)array('error'=> true, 'user' => esc_html__('User name null', 'bravis-user'))));

            /* if pass null. */
            if(empty($login_data['pass']))
                die(json_encode((object)array('error'=> true, 'pass' => esc_html__('Password null', 'bravis-user'))));

            /* rememberme */
            $login_data->rememberme = (isset($login_data['rememberme']) && $login_data['rememberme']) ? true : false;

            /* login. */
            if($this->login_action($login_data['user'], $login_data['pass'], $login_data['rememberme']))
                die(json_encode((object)array('error'=> false)));

            /* error */
            if(username_exists($login_data['user']))
                die(json_encode((object)array('error'=> true, 'pass'=> esc_html__('Password incorrect', 'bravis-user'))));

            die(json_encode((object)array('error'=> true, 'user'=> esc_html__('User incorrect', 'bravis-user'))));
        }

        /**
         * Login action
         **/
        function login_action($user, $pass, $rememberme = false)
        {
            /* check user & pass */
            if (! wp_login($user, $pass))
                return false;

            /* get user by name. */
            $user = get_user_by('slug', $user);

            /* set login. */
            wp_set_auth_cookie( $user->data->ID, $rememberme, false );

            return true;
        }

         
    }
    
    new Bravis_User_Ajax();
}